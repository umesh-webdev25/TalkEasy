      ...
      >
        ...
        {time && (
          <span
            className={cn(
              "block mt-1 text-[9px]",
              isAI ? "text-app-text-muted" : "opacity-60",
            )}
          >
            {time}
          </span>
...
export const ShowcaseChatScreen = memo(function ShowcaseChatScreen({ className }) {
  return (
    <div
      className={cn(
...
export const VoiceCallScreen = memo(function VoiceCallScreen({ className }) {
  return (
    <div
      className={cn(
        "relative h-full flex flex-col items-center justify-center gap-6 p-6 overflow-hidden",
        className,
      )}
    >
      ...
      <div className="relative flex flex-col items-center gap-5">
        ...
        <div className="text-center">
          ...
          <p className="text-[10px] text-app-text-muted mt-0.5 font-medium">
            Live · 16 kHz · studio quality
          </p>
        </div>
        <div
          className="flex items-end justify-center gap-1 h-10 text-app-text"
          aria-hidden
        >
        ...
        <div className="flex items-center gap-3">
          <span className="w-11 h-11 rounded-full border border-[var(--glass-border)] bg-[var(--glass-bg)] flex items-center justify-center text-[11px]">
            🔇
          </span>
          <span className="w-14 h-14 rounded-full bg-[var(--text-primary)] text-[var(--bg-app)] flex items-center justify-center text-[11px]">
            📞
          </span>
          <span className="w-11 h-11 rounded-full border border-[var(--glass-border)] bg-[var(--glass-bg)] flex items-center justify-center text-[11px]">
            ✎
...
export const ImageGenScreen = memo(function ImageGenScreen({ className }) {
  ...
  return (
    <div
      className={cn(
        "flex flex-col gap-3 p-4 h-full overflow-hidden",
        className,
      )}
    >
      <div className="flex items-center gap-2 rounded-xl border border-[var(--glass-border)] bg-[var(--glass-bg)] px-3.5 py-2.5">
        <span className="text-[11px] text-app-text flex-1 truncate">
          a minimal 3D isometric workspace with soft shadows
        </span>
        <span className="text-[10px] font-semibold px-2.5 py-1 rounded-lg bg-[var(--text-primary)] text-[var(--bg-app)] shrink-0">
          Generate
        </span>
      </div>
...
export const PdfChatScreen = memo(function PdfChatScreen({ className }) {
  return (
    <div
      className={cn(
        "grid grid-cols-[1fr_1.6fr] h-full overflow-hidden",
        className,
      )}
    >
      <div className="border-r border-[var(--glass-border)] p-3 flex flex-col gap-2 overflow-hidden">
        <p className="text-[10px] font-semibold text-app-text-muted px-1">
          Documents
        </p>
        {[
          ...
          <div
            key={doc.name}
            className={cn(
              "flex items-center gap-2.5 rounded-lg border border-[var(--glass-border)] bg-[var(--glass-bg)] px-2.5 py-2",
              i === 0 && "ring-1 ring-[var(--text-primary)]",
            )}
          >
            <span className="w-6 h-7 rounded bg-red-500/15 border border-red-500/25 flex items-center justify-center text-[9px]">
              PDF
            </span>
            <span className="min-w-0">
              ...
              <span className="block text-[8px] text-app-text-muted">
                {doc.pages}
              </span>
            </span>
          </div>
      ...
      <div className="flex flex-col gap-2.5 p-3.5 overflow-hidden">
        <Bubble from="user" time="14:02">
          What are the key findings in chapter 2?
        </Bubble>
        <Bubble from="ai" time="14:02">
          Chapter 2 outlines three findings: adoption grew 41% quarter over
          quarter, retention improved 12%, and the APAC region is your
          fastest-growing segment.
        </Bubble>
        <Bubble from="user">
          Can you quote the exact sentence on pricing?
        </Bubble>
        <Bubble from="ai">
          "The annual plan includes unlimited voice minutes and priority GPU
          access." — §2.4
        </Bubble>
      </div>
    </div>
