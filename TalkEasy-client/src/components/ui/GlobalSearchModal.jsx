      ...
      <span>
        {parts.map((part, i) =>
          part.toLowerCase() === highlight.toLowerCase() ? (
            <span
              key={i}
              className="bg-brand-blue/30 text-brand-blue dark:text-brand-cyan px-1 rounded"
            >
              {part}
            </span>
          ) : (
    ...
    >
      <div className="flex flex-col h-[300px] ">
        ...
        <div className="relative mb-4 shrink-0 group">
          <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-app-text-muted group-focus-within:text-brand-blue transition-colors duration-300">
            <Search size={17} strokeWidth={2.25} />
          </div>
          <input
            ref={inputRef}
            type="text"
            className="w-full bg-glass-input-bg border border-glass-border rounded-2xl py-2.5 pl-10 pr-9 text-sm outline-none text-app-text placeholder:text-app-text-muted/70 shadow-sm focus:ring-4 focus:ring-brand-blue/15 focus:border-brand-blue/50 hover:border-glass-border/80 transition-all duration-300"
          ...
          {query && (
            <button
              onClick={() => setQuery("")}
              className="absolute inset-y-0 right-0 pr-3 flex items-center text-app-text-muted hover:text-app-text hover:scale-110 active:scale-95 transition-all duration-200"
            >
              <X size={15} />
            </button>
          )}
